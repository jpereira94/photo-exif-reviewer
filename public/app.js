...
const folderButtons = [
  ...document.querySelectorAll(
    "#choose-folder, #welcome-choose-folder, #empty-choose-folder",
  ),
];
...
// O #photo usa object-fit: contain, por isso a imagem pintada é menor do que a
// caixa do elemento sempre que as proporções não coincidem. A grelha é colocada
// sobre a área pintada, não sobre a caixa.
function updateThirds() {
  const visible =
    showThirds && !compareMode && !reviewer.hidden && photo.naturalWidth > 0;
  thirds.hidden = !visible;
  ...
  const scale = Math.min(
    photoBox.width / photo.naturalWidth,
    photoBox.height / photo.naturalHeight,
  );
  const width = photo.naturalWidth * scale;
  const height = photo.naturalHeight * scale;

  thirds.style.left = `${photoBox.left - stageBox.left + (photoBox.width - width) / 2}px`;
  thirds.style.top = `${photoBox.top - stageBox.top + (photoBox.height - height) / 2}px`;
  thirds.style.width = `${width}px`;
  thirds.style.height = `${height}px`;
}

function setThirds(value) {
  showThirds = value;
  gridToggle.setAttribute("aria-pressed", String(showThirds));

  try {
    localStorage.setItem("photo-reviewer:thirds", showThirds ? "1" : "0");
  } catch {
...
function orderedPhotos() {
  ...
  if (sortMode === "score") {
    list = [...list].sort(
      (left, right) =>
        (analysis[right.name]?.score ?? -1) -
        (analysis[left.name]?.score ?? -1),
    );
    ...
    list = [...list].sort((left, right) => {
      ...
      return (
        leftGroup.groupId - rightGroup.groupId ||
        rightGroup.score - leftGroup.score
      );
    });
  }
...
function navigationIndices() {
  return displayIndices().filter(
    (index) => !selectedOnly || selectedNames.has(photos[index]?.name),
  );
}
...
function updateControls() {
  ...
  selectedOnlyButton.textContent = selectedCount
    ? `Só selecionadas (${selectedCount})`
    : "Só selecionadas";
  ...
  compareButton.textContent =
    selectedCount >= 2 ? `Comparar (${selectedCount})` : "Comparar";
  ...
  bestOnlyButton.textContent = analysed.length
    ? `Só as melhores (${bestCount})`
    : "Só as melhores";
...
function createThumbnail(item) {
  ...
  if (entry) {
    ...
    badge.textContent =
      entry.isGroupBest && entry.groupSize > 1
        ? `★ ${entry.score}`
        : String(entry.score);
    badge.title =
      entry.groupSize > 1
...
function createGroupBox(members) {
  ...
  if (!entry || entry.groupSize < 2) {
    ...
    const rest = members.filter(
      (member) => !analysis[member.name]?.isGroupBest,
    );
    const allSelected =
      rest.length > 0 && rest.every((member) => selectedNames.has(member.name));
    selectRest.type = "button";
    selectRest.className = "thumbnail-group-action";
    selectRest.textContent = allSelected
      ? "Desmarcar repetidas"
      : "Selecionar repetidas";
...
function renderThumbnails() {
  ...
  document
    .querySelector(`.thumbnail[data-index="${currentIndex}"]`)
    ?.scrollIntoView({
      block: "nearest",
      inline: "nearest",
    });
...
function renderAnalysisStatus() {
  analysisStatus.hidden =
    !folder || (!analysisRunning && !Object.keys(analysis).length);
...
async function pollAnalysis() {
  ...
  try {
    const data = await request(
      `/api/analysis?sensitivity=${encodeURIComponent(sensitivity)}`,
    );
...
async function deletePhoto() {
  ...
  try {
    const result = await request(`/api/photos/${currentIndex}`, {
      method: "DELETE",
    });
    ...
    showToast(
      `Movido${deletedFiles.length > 1 ? "s" : ""} para o Lixo: ${deletedFiles.join(", ")}.`,
    );
    ...
    if (selectedOnly) {
      const firstSelected = photos.findIndex((photoItem) =>
        selectedNames.has(photoItem.name),
      );
...
gridToggle.addEventListener("click", () => setThirds(!showThirds));
photo.addEventListener("load", updateThirds);
window.addEventListener("resize", updateThirds);

try {
  setThirds(localStorage.getItem("photo-reviewer:thirds") !== "0");
} catch {
  setThirds(true);
}
...
document
  ...
  .addEventListener("click", () => changeZoom(zoomScale * 1.25));
document
...
zoomStage.addEventListener(
  ...
  (event) => {
    event.preventDefault();
    changeZoom(
      zoomScale * (event.deltaY < 0 ? 1.15 : 1 / 1.15),
      event.clientX,
      event.clientY,
    );
  },
...
// Só a escrita de texto rouba os atalhos. As caixas de seleção e os botões das
// miniaturas ficam com o foco depois de um clique, e aí o espaço tem de ser o
// atalho de seleção em vez de reativar o elemento focado.
...
window.addEventListener("keydown", (event) => {
  if (reviewer.hidden || compareMode || isTextField(document.activeElement)) {
    return;
  }
  ...
  if (event.key === "ArrowLeft") {
    ...
    // Os modificadores são ignorados só aqui: o Cmd+Space é do sistema, enquanto
    // o Cmd+Backspace tem de continuar a apagar como sempre apagou.
    // Sem o preventDefault o espaço faz scroll à página e reativa o botão focado.
    event.preventDefault();

    if (photos[currentIndex]) {
      toggleSelection(photos[currentIndex].name);
    }
